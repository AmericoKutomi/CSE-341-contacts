...
const createContact = async (req, res) => {
  ...
  const contact = {
    ...
    if (response.acknowledged) {
      ...
      res.status(500).json(response.error || 'Some error ocurred while creating a contact');     
    }
};
...
const updateContact = async (req, res) => {
  ...
  const contactId = new ObjectId(req.params.id);
    ...
    if (response.deletedCount > 0) {
      ...
      res.status(500).json(response.error || 'Some error ocurred while deleting a contact');     
    }
  };
